...
function MiniPill({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <span className={`inline-flex items-center justify-center rounded-full border border-white/15 bg-white/10 px-2.5 py-1 text-xs font-medium text-white/90 backdrop-blur ${className}`}>
      {children}
    </span>
  )
}
        ...
        <AnimatePresence initial={false}>
          {!isSearching && featuredRecipe ? (
            <motion.header
              ...
              className="relative overflow-hidden rounded-[1.75rem] sm:rounded-[2.5rem] bg-stone-950 p-4 sm:p-5 text-white shadow-2xl shadow-stone-950/20 md:p-8"
            >
              ...
              <div className="relative flex h-full flex-col justify-between gap-6 sm:gap-10">
                <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3">
                  <div className="flex gap-2">
                    <MiniPill>Newest recipe</MiniPill>
                ...
                <div className="max-w-2xl">
                  ...
                  <div className="mt-4 sm:mt-8 flex flex-wrap gap-2">
                    ...
                    {featuredRecipe.publisher && (
                      <MiniPill>By {featuredRecipe.publisher.nickname || 'Umami cook'}</MiniPill>
                    )}
                  </div>
                </div>
              ...
              <AnimatePresence mode="popLayout">
                {error ? (
                  ...
                  >
                    ...
                    <button onClick={() => setQuery('')} className="mt-3 sm:mt-4 text-xs sm:text-sm font-medium text-stone-950 underline underline-offset-4">Reset search</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
